...
class Butterworth(object):
    ...
    @staticmethod
    def butter_high_filter(data, highcut, fs, order=4):
        b, a = Butterworth._high_(highcut, fs, order=order)
        y = filtfilt(b, a, data)
        return y

    @staticmethod
    def butter_low_filter(data, highcut, fs, order=4):
        b, a = Butterworth._low_(highcut, fs, order=order)
        y = filtfilt(b, a, data)
        return y

    @staticmethod
    def butter_low_pass(data, para):
