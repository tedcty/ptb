...
class Reader(Manager):
    ...
    def __init__(self, handle):
        ...
        if processor != PROCESSOR_INTEL:
            raise ValueError('we only read Intel C3D files (got processor {})'
                             .format(processor))
    ...
    def read_frames(self, copy=True):
        ...
        offsets = np.zeros((1, self.analog_used), int)
        param = self.get('ANALOG:OFFSET')
        if param is not None:
            offsets = param.int16_array[None, :self.analog_used]

        scales = np.ones((1, self.analog_used), float)
        param = self.get('ANALOG:SCALE')
        if param is not None:
            scales = param.float_array[None, :self.analog_used]
        ...
        for frame_no in range(self.first_frame(), self.last_frame() + 1):
            ...
            if self.header.analog_count > 0:
                ...
                raw = np.frombuffer(self._handle.read(n * analog_bytes),
                                    dtype=analog_dtype,
                                    count=n).reshape((-1, self.analog_used))
